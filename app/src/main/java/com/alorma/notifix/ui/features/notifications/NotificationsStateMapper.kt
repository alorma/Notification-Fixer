...
class NotificationsStateMapper @Inject constructor() {
    fun mapSuccessState(items: List<AppNotification>) = ShowNotifications(items.map {
        NotificationViewModel(it.id, it.title, it.text, it.checked, it.color)
    })

    fun mapUpdate(it: NotificationViewModel) = AppNotification(it.id, it.title, it.text, it.color, it.checked)
}